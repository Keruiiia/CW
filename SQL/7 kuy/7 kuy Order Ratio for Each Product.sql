...
The orders table represents all orders made in an online store.
Each row represents a single order for a product, so the same product
could have multiple orders and therefore appear multiple times in the table.
...
ratio: The ratio of number of orders for this product to number of all
orders in the table, represented as a percentage. This should be of numeric type, rounded to 2 decimal places
...
SELECT product_id,
  COUNT(*) AS count,
  ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM orders), 2) AS ratio
FROM orders
GROUP BY product_id
ORDER BY product_id;