...
film and in the brackets, its rating, rental count, and the date of the most recent
rental. Since the business is in America, please use American date notation (for example, 'April 09, 2023').

Notes:
Sort by count of rented films from higher to lower. If two or more films have
the same amount of rentals then sort by last rental date in descending order,
if even last rental dates are the same - then by film title alphabetically.
"in the past month" - interval inclusive of 1 month from CURRENT_DATE. 
We are not taking time part of today into account for this task
